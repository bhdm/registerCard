...
//                'constructorArgs' => array('utf-8', 'A4', 5 ,5 ,5 ,5,5 ), //Constructor arguments. Numeric array. Don't forget about points 2 and 3 in Warning section!
    ...
    /**
     * @Route("/generatePaymentFastPdf", name="generate_payment_fast_pdf")
     */
    public function generatePaymentFastPdfAction(Request $request){
        if ($request->query->get('ord')!=null){
            $userId = $request->query->get('ord');
            $user = $this->getDoctrine()->getRepository('CrmMainBundle:FastOrder')->findOneById($userId);
            $mpdfService = $this->container->get('tfox.mpdfport');
            $html = $this->render('CrmMainBundle:Form:payment_doc_fast.html.twig',array('user' => $user));
//            return $html;
            $arguments = array(
//                'constructorArgs' => array('utf-8', 'A4', 5 ,5 ,5 ,5,5 ), //Constructor arguments. Numeric array. Don't forget about points 2 and 3 in Warning section!
                'writeHtmlMode' => null, //$mode argument for WriteHTML method
