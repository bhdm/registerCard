...
//SELECT c.id, c.title, SUM(q.quota) sumQuota, SUM(q2.quota) sumQuota2, SUM(u.price) sumPrice
    public function debtors(){
        $sql = "
            ...
            LEFT JOIN user u ON u.company_id = c.id  AND u.enabled =1
            AND u.status !=0
            AND u.status !=1
            AND u.status !=10
    ...
    public function debtors3(){
        $sql = "
            SELECT c.id, c.title,
            ((SELECT SUM(q.quota) FROM CompanyQuotaLog q WHERE  q.enabled =1 AND q.company_id = c.id )) s0, 
            ((SELECT SUM(u.price) FROM user u WHERE  u.enabled =1
            AND u.status !=0
            AND u.status !=1
            AND u.status !=10
            AND  u.company_id = c.id 
             ))  s1,
            ((SELECT SUM(u2.price) FROM companyUser u2 WHERE  u2.enabled =1
            AND u2.status !=0        
            AND u2.status !=10
