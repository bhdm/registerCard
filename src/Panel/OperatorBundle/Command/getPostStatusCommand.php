...
//    https://www.pochta.ru/tracking?p_p_id=trackingPortlet_WAR_portalportlet&p_p_lifecycle=2&p_p_state=normal&p_p_mode=view&p_p_resource_id=getList&p_p_cacheability=cacheLevelPage&p_p_col_id=column-1&p_p_col_count=1&barcodeList=19010395015457&postmanAllowed=true&_=234234
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $orders = $em->createQuery("
            ...
            WHERE u.isProduction is NULL AND u.status != 10 AND u.enabled = 1 AND u.created > '".$date->format('Y-m-d H:i:s')."'")->getResult();
        $txt = '';
        foreach ($orders as $order){
            if ($order['post']){
                $post = $order['post'];
                $json = "https://www.pochta.ru/tracking?p_p_id=trackingPortlet_WAR_portalportlet&p_p_lifecycle=2&p_p_state=normal&p_p_mode=view&p_p_resource_id=getList&p_p_cacheability=cacheLevelPage&p_p_col_id=column-1&p_p_col_count=1&barcodeList=$post&postmanAllowed=true";
                $output->writeln($json['list'][0]['trackingItem']['hasBeenGiven']);
